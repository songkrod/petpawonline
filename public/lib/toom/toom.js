...
// Private Methods
	...
	function buildOut() {
		this.wrap = document.getElementsByClassName(this.options.selector);

		for (var i = 0; i < this.wrap.length; i++) {

			var computedStyle = window.getComputedStyle(this.wrap[i]);
			var wrapInfo = {
			...
			var items = this.wrap[i].getElementsByClassName(this.options.itemSelector);
			...
			if (items.length > 0) {
				...
				for (var j = 0; j < items.length; j++) {
					...
					items[j].style.top = pos.top[tmpIndex];
					items[j].style.left = pos.left[tmpIndex];

					pos.top[tmpIndex] += items[j].offsetHeight + parseInt(this.options.gap);
				}

				this.wrap[i].style.height = indexOfMax(pos.top).value + space.bottom;
			}
		}
	}
